...
PLINK_DIR="$HOME/donglab/data/target_ALS/QTL/plink"
RAW="$PLINK_DIR/joint_all_chrs_matrixEQTL.raw"
BIM="$PLINK_DIR/joint_all_chrs_filtered_bed.bim"
GWAS="$HOME/donglab/data/GCST90027163/GWAS/harmonised/34873335-GCST90027163-MONDO_0004976.h.tsv.gz"
OUTROOT="$HOME/donglab/data/GCST90027163/GWAS/eQTL_harmonization"
SUMMARY="$OUTROOT/all_tissues_retention_summary.tsv"
...
raw=os.environ["RAW"]; bim_path=os.environ["BIM"]; gwas_path=os.environ["GWAS"]
outroot=os.environ["OUTROOT"]; summary_path=os.environ["SUMMARY"]
tissues=os.environ["TISSUE_LIST"].split()
base=os.path.expanduser("~/donglab/data/target_ALS")
meta_cols={"FID","IID","PAT","MAT","SEX","PHENOTYPE"}
comp={"A":"T","T":"A","C":"G","G":"C"}

def norm_chr(s):
    return s.astype(str).str.replace(r"^chr","",regex=True,case=False).str.upper()
...
variant_meta["other_allele"]=variant_meta.apply(
    lambda r:r["bim_a2"] if r["effect_allele"]==r["bim_a1"] else r["bim_a1"] if r["effect_allele"]==r["bim_a2"] else None,
    axis=1
)
variant_meta["allele_status"]=variant_meta.apply(
    lambda r:"ok" if r["effect_allele"] in {r["bim_a1"],r["bim_a2"]} else "missing_bim_match" if pd.isna(r["bim_id"]) else "counted_allele_not_in_bim",
    axis=1
)
...
gwas=pd.read_csv(
    gwas_path,sep="\t",compression="gzip",usecols=gwas_cols,
    dtype={"hm_rsid":str,"hm_chrom":str,"hm_pos":str,"hm_other_allele":str,"hm_effect_allele":str}
)
gwas=gwas.rename(columns={
    "hm_rsid":"snpid","hm_chrom":"gwas_chr","hm_pos":"gwas_pos",
    "hm_other_allele":"gwas_other_allele","hm_effect_allele":"gwas_effect_allele",
    "hm_beta":"gwas_beta_original","standard_error":"gwas_se","p_value":"gwas_p"
})
gwas["gwas_effect_allele"]=gwas["gwas_effect_allele"].str.upper()
gwas["gwas_other_allele"]=gwas["gwas_other_allele"].str.upper()
gwas["gwas_beta_original"]=pd.to_numeric(gwas["gwas_beta_original"],errors="coerce")
gwas["gwas_se"]=pd.to_numeric(gwas["gwas_se"],errors="coerce")
...
for tissue in tissues:
    eqtl_dir=f"{base}/{tissue}/eQTL"
    leads_path=f"{eqtl_dir}/results/{tissue}_eQTL.lead_snps.txt"
    loc_path=f"{eqtl_dir}/snp_location.txt"
    tissue_out=f"{outroot}/{tissue}"
    allele_out=f"{eqtl_dir}/qtl_alleles_GRCh38.tsv"
    all_out=f"{tissue_out}/{tissue}_eQTL_GWAS_all.tsv.gz"
    kept_out=f"{tissue_out}/{tissue}_eQTL_GWAS_MR_ready.tsv.gz"
    rejected_out=f"{tissue_out}/{tissue}_eQTL_GWAS_rejected.tsv.gz"

    os.makedirs(tissue_out,exist_ok=True)
    for f in [leads_path,loc_path]:
        if not os.path.isfile(f) or os.path.getsize(f)==0:
            raise FileNotFoundError(f"Missing or empty file: {f}")
    ...
    leads["beta"]=pd.to_numeric(leads["beta"],errors="coerce")
    leads["t-stat"]=pd.to_numeric(leads["t-stat"],errors="coerce")
    leads["p-value"]=pd.to_numeric(leads["p-value"],errors="coerce")
    leads["se_qtl"]=(leads["beta"]/leads["t-stat"]).abs()

    lead_rows=len(leads)
    lead_ids=set(leads["snpid"].dropna())
    unique_lead_snps=len(lead_ids)

    tissue_variants=variant_meta.merge(loc[["key","final_snpid","allele_chr","allele_pos"]],on="key",how="left")
    alleles=tissue_variants[tissue_variants["final_snpid"].isin(lead_ids)][
        ["final_snpid","allele_chr","allele_pos","effect_allele","other_allele","bim_a1","bim_a2","raw_variant_id","allele_status"]
    ].copy()
    ...
    qtl=leads.merge(alleles,on="snpid",how="left",validate="many_to_one")
    x=qtl.merge(gwas,on="snpid",how="left",validate="many_to_one")
    ...
    x["effect_allele"]=x["effect_allele"].str.upper()
    x["other_allele"]=x["other_allele"].str.upper()
    ...
    x["harmonized_effect_allele"]=x["effect_allele"]
    x["harmonized_other_allele"]=x["other_allele"]
    ...
    allele_rows=int(qtl["effect_allele"].notna().sum())
    matched=int(x["found_in_gwas"].sum())
    exact=int((x["harmonization_relation"]=="exact").sum())
    swapped=int((x["harmonization_relation"]=="swap_beta").sum())
    strand=int((x["harmonization_relation"]=="strand").sum())
    strand_swap=int((x["harmonization_relation"]=="strand_and_swap_beta").sum())
    palindromic=int((x["found_in_gwas"]&x["palindromic"]).sum())
    missing_gwas=int((~x["found_in_gwas"]).sum())
    retained=int(x["harmonization_keep"].sum())

    summaries.append({
        "tissue":tissue,
        "lead_gene_snp_rows":lead_rows,
        "unique_lead_snps":unique_lead_snps,
        "rows_with_alleles":allele_rows,
        "matched_in_GWAS":matched,
        "exact":exact,
        "swap_beta":swapped,
        "strand":strand,
        "strand_and_swap_beta":strand_swap,
        "matched_palindromic":palindromic,
        "not_found_in_GWAS":missing_gwas,
        "retained_MR_ready":retained,
        "allele_recovery_pct":pct(allele_rows,lead_rows),
        "GWAS_match_pct":pct(matched,lead_rows),
        "retention_pct":pct(retained,lead_rows)
    })

    print(f"Lead gene-SNP rows: {lead_rows:,}")
    print(f"Unique lead SNPs: {unique_lead_snps:,}")
    print(f"Rows with alleles: {allele_rows:,} ({pct(allele_rows,lead_rows):.2f}%)")
    print(f"Matched in GWAS: {matched:,} ({pct(matched,lead_rows):.2f}%)")
    print(f"Exact: {exact:,}")
    print(f"GWAS beta reversed: {swapped+strand_swap:,}")
    print(f"Strand-only matches: {strand:,}")
    print(f"Palindromic excluded: {palindromic:,}")
    print(f"Not found in GWAS: {missing_gwas:,}")
    print(f"MR-ready retained: {retained:,} ({pct(retained,lead_rows):.2f}%)")
