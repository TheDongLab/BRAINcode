...
OUT_VCF="${OUT_DIR}/TargetALS_chrX_first20_Females.vcf.gz"

module load BCFtools/1.21

# 1. Create a temporary list of the first 20 female IDs
head -n 20 "$FEMALES_SRC" > "${OUT_DIR}/top20_female_ids.txt"

# 2. Extract, Normalize, and Zip
# We include 'chrX' to limit the scope and 'norm' to ensure the VCF is clean
bcftools view -S "${OUT_DIR}/top20_female_ids.txt" "$VCF_IN" chrX | \
