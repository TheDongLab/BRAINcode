...
for chr in {1..22} X Y; do
    ...
    if [ "$chr" == "X" ]; then
        ...
        # 2. Re-code male genotypes safely
        # We check if the genotype is missing (contains .) - if so, we leave it as .
        # If not, we strip the slash to make it haploid.
        ...
        }
        {
            for (i=10; i<=NF; i++) {
                if (male_col[i]) {
                    if ($i ~ /\./) { $i = "." } # Handle missing data correctly
                    else { gsub(/[\/|]/, "", $i) } # Strip slashes for actual genotypes
                }
            }
            print $0;
