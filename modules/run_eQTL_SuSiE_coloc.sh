...
GWAS_ORIG="$HOME/donglab/data/GCST90027163/GWAS/GCST90027163_buildGRCh37.tsv.gz"
...
for(tissue in TISSUES){
  ...
  snpfile<-file.path(mrdir,paste0(tissue,"_SNP_results.tsv.gz"))
  relfile<-file.path(mrdir,paste0(tissue,"_SNP_gene_effects.tsv.gz"))
  outroot<-file.path(mrdir,"SuSiE_coloc")

  # Start each run clean so old gene directories cannot contaminate results.
  if(dir.exists(outroot)) unlink(outroot,recursive=TRUE,force=TRUE)
  ...
  for(f in c(snpfile,relfile,fullfile,locfile,covfile))
    if(!file.exists(f)||file.info(f)$size==0) stop("Missing/empty: ",f)

  # ============================================================
  # UNIQUE SNP SIGNIFICANCE FIRST
  # ============================================================
  snps<-fread(snpfile)
  ...
  cand_snps<-switch(MODE,
    global_fdr=snps[is.finite(FDR_ALS_global)&FDR_ALS_global<0.05],
    tissue_fdr=snps[is.finite(FDR_ALS_tissue)&FDR_ALS_tissue<0.05],
    nominal=snps[is.finite(gwas_p)&gwas_p<0.05],
    all=copy(snps),
  ...
  # ============================================================
  # ATTACH EVERY REGULATORY GENE OF THOSE SNPs
  # ============================================================
  rel<-fread(relfile)
  cand<-rel[snpid%in%cand_snps$snpid]
  cand<-merge(cand,cand_snps[,.(snpid,gwas_p,FDR_ALS_tissue,FDR_ALS_global)],
              by="snpid",all.x=TRUE,suffixes=c("","_SNP"))

  # merge may duplicate columns already present in effects table;
  # always use values from unique-SNP result table.
  for(c in c("gwas_p","FDR_ALS_tissue","FDR_ALS_global")){
    cs<-paste0(c,"_SNP")
    if(cs%in%names(cand)){cand[,(c):=get(cs)]; cand[,(cs):=NULL]}
  }
  ...
  cat("\n",strrep("=",70),"\n",tissue,
      ...
      nrow(cand)," SNP×gene relationships → ",
      length(genes)," candidate genes\n",strrep("=",70),"\n",sep="")
  ...
  for(gene in genes){
    ...
    q[,`:=`(snpid=as.character(snpid),qtl_chr=nch(chr),qtl_pos=as.integer(pos),
             beta_qtl=as.numeric(beta),se_qtl=abs(as.numeric(beta)/as.numeric(`t-stat`)),p_qtl=as.numeric(`p-value`))]
    ...
    if(n_locus<MINSHARED){
      ...
      master[[length(master)+1]]<-data.table(
        tissue,geneid=gene,status="too_few_locus_snps",n_locus=n_locus,n_shared=0,shared_fraction=0,
    ...
    if(nrow(x)<MINSHARED||shared_frac<MINFRAC){
      ...
      master[[length(master)+1]]<-data.table(
        tissue,geneid=gene,status,n_locus=n_locus,n_shared=nrow(x),shared_fraction=shared_frac,
        n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }
    ...
    if(!file.exists(trawfile)){
      cat("PLINK failed\n")
      master[[length(master)+1]]<-data.table(
        tissue,geneid=gene,status="PLINK_failed",n_locus=n_locus,n_shared=nrow(x),shared_fraction=shared_frac,
        n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }
    ...
    if(nrow(tr)<MINSHARED||!length(geno_cols)){
      ...
      master[[length(master)+1]]<-data.table(
        tissue,geneid=gene,status="too_few_PLINK_snps",n_locus=n_locus,n_shared=nrow(x),shared_fraction=shared_frac,
        n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }
    ...
    if(nrow(x)<MINSHARED){
      ...
      master[[length(master)+1]]<-data.table(
        tissue,geneid=gene,status="too_few_variable_snps",n_locus=n_locus,n_shared=nrow(x),shared_fraction=nrow(x)/n_locus,
        n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }
    ...
    ans<-tryCatch({
      ...
      status=paste0("ERROR: ",conditionMessage(e)),h4=NA_real_,ncs1=NA_integer_,ncs2=NA_integer_,
    ...
    cat(ans$status,", shared=",nrow(x),"/",n_locus,
...
# PP.H4 HISTOGRAM
# ==============================================================
h4<-res[is.finite(max_PP_H4),.(tissue,geneid,max_PP_H4,n_candidate_SNPs,best_candidate_SNP,best_ALS_p,best_ALS_global_FDR)]
