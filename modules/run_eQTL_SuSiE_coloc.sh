...
TISSUES<-strsplit(Sys.getenv("TISSUE_LIST")," ")[[1]]; MODE<-Sys.getenv("CANDIDATE_MODE"); H4REF<-as.numeric(Sys.getenv("H4_REFERENCE")); MINSHARED<-as.integer(Sys.getenv("MIN_SHARED_SNPS")); MINFRAC<-as.numeric(Sys.getenv("MIN_SHARED_FRAC")); L<-as.integer(Sys.getenv("SUSIE_L"))
...
amap<-merge(rawmap,bim[,.(rawid,key,a1=toupper(a1),a2=toupper(a2))],by="rawid",all.x=TRUE); amap[,other_allele:=fifelse(effect_allele==a1,a2,fifelse(effect_allele==a2,a1,NA_character_))]
...
setnames(gwas,c("hm_rsid","hm_chrom","hm_pos","hm_other_allele","hm_effect_allele","hm_beta","standard_error","p_value"),c("snpid","gwas_chr","gwas_pos","gwas_oa","gwas_ea","gwas_beta","gwas_se","gwas_p"))
gwas[,`:=`(snpid=as.character(snpid),gwas_chr=nch(gwas_chr),gwas_pos=as.integer(gwas_pos),gwas_ea=toupper(gwas_ea),gwas_oa=toupper(gwas_oa),gwas_beta=as.numeric(gwas_beta),gwas_se=as.numeric(gwas_se),gwas_p=as.numeric(gwas_p))]
...
for(tissue in TISSUES){
  mrdir<-file.path(BASE,tissue,"MR"); snpfile<-file.path(mrdir,paste0(tissue,"_SNP_results.tsv.gz")); relfile<-file.path(mrdir,paste0(tissue,"_SNP_gene_effects.tsv.gz")); outroot<-file.path(mrdir,"SuSiE_coloc")
  if(dir.exists(outroot)) unlink(outroot,recursive=TRUE,force=TRUE); dir.create(outroot,recursive=TRUE,showWarnings=FALSE)

  fullfile<-file.path(BASE,tissue,"eQTL","results",paste0(tissue,"_eQTL.full_annotated.txt")); locfile<-file.path(BASE,tissue,"eQTL","snp_location.txt"); covfile<-file.path(BASE,tissue,"eQTL",paste0("covariates_",tissue,"_encoded.txt"))
  for(f in c(snpfile,relfile,fullfile,locfile,covfile)) if(!file.exists(f)||file.info(f)$size==0) stop("Missing/empty: ",f)
  ...
  cand_snps<-switch(MODE,global_fdr=snps[is.finite(FDR_ALS_global)&FDR_ALS_global<.05],tissue_fdr=snps[is.finite(FDR_ALS_tissue)&FDR_ALS_tissue<.05],nominal=snps[is.finite(gwas_p)&gwas_p<.05],all=copy(snps),stop("Unknown CANDIDATE_MODE: ",MODE))
  if(!nrow(cand_snps)){cat("\n",tissue,": zero candidate SNPs\n",sep=""); next}

  rel<-fread(relfile); cand<-rel[snpid%in%cand_snps$snpid]
  cand<-merge(cand,cand_snps[,.(snpid,gwas_p,FDR_ALS_tissue,FDR_ALS_global)],by="snpid",all.x=TRUE,suffixes=c("","_SNP"))
  for(c in c("gwas_p","FDR_ALS_tissue","FDR_ALS_global")){cs<-paste0(c,"_SNP"); if(cs%in%names(cand)){cand[,(c):=get(cs)]; cand[,(cs):=NULL]}}
  if(!nrow(cand)){cat("\n",tissue,": candidate SNPs have no regulatory targets\n",sep=""); next}

  gene_meta<-cand[,{
    z<-unique(.SD[,.(snpid,gwas_p,FDR_ALS_tissue,FDR_ALS_global)])[order(gwas_p)]
    .(n_candidate_SNPs=uniqueN(snpid),candidate_SNPs=paste(sort(unique(snpid)),collapse=";"),best_candidate_SNP=as.character(z$snpid[1]),best_ALS_p=as.numeric(z$gwas_p[1]),best_ALS_tissue_FDR=min(z$FDR_ALS_tissue,na.rm=TRUE),best_ALS_global_FDR=min(z$FDR_ALS_global,na.rm=TRUE))
  },by=geneid]
  gene_meta[!is.finite(best_ALS_tissue_FDR),best_ALS_tissue_FDR:=NA_real_]; gene_meta[!is.finite(best_ALS_global_FDR),best_ALS_global_FDR:=NA_real_]
  genes<-gene_meta$geneid

  cat("\n",strrep("=",70),"\n",tissue,": ",nrow(cand_snps)," unique candidate SNPs → ",nrow(cand)," SNP×gene relationships → ",length(genes)," candidate genes\n",strrep("=",70),"\n",sep="")
  ...
  for(gene in genes){
    ...
    q[,`:=`(snpid=as.character(snpid),qtl_chr=nch(chr),qtl_pos=as.integer(pos),beta_qtl=as.numeric(beta),se_qtl=abs(as.numeric(beta)/as.numeric(`t-stat`)),p_qtl=as.numeric(`p-value`))]
    q<-q[!is.na(snpid)&is.finite(beta_qtl)&is.finite(se_qtl)&se_qtl>0]; q<-merge(q[,.(geneid,snpid,qtl_chr,qtl_pos,beta_qtl,se_qtl,p_qtl)],loc[,.(snpid,key)],by="snpid",all.x=TRUE)
    q<-merge(q,amap,by="key",all.x=TRUE); q<-unique(q,by="snpid"); n_locus<-nrow(q)

    if(n_locus<MINSHARED){
      cat("too few locus SNPs (",n_locus,")\n",sep="")
      master[[length(master)+1]]<-data.table(tissue,geneid=gene,status="too_few_locus_snps",n_locus=n_locus,n_shared=0,shared_fraction=0,n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }
    ...
    x[,relation:=fcase(effect_allele==gwas_ea&other_allele==gwas_oa,"exact",effect_allele==gwas_oa&other_allele==gwas_ea,"swap",effect_allele==comp[gwas_ea]&other_allele==comp[gwas_oa],"strand",effect_allele==comp[gwas_oa]&other_allele==comp[gwas_ea],"strand_swap",default="bad")]
    x<-unique(x[!pal&relation!="bad"&is.finite(gwas_beta)&is.finite(gwas_se)&gwas_se>0],by="snpid"); x[,gwas_beta_h:=fifelse(relation%in%c("swap","strand_swap"),-gwas_beta,gwas_beta)]
    ...
    if(nrow(x)<MINSHARED||shared_frac<MINFRAC){
      ...
      master[[length(master)+1]]<-data.table(tissue,geneid=gene,status,n_locus=n_locus,n_shared=nrow(x),shared_fraction=shared_frac,n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }

    idsfile<-file.path(gout,"raw_ids.txt"); fwrite(x[,.(rawid)],idsfile,col.names=FALSE); pref<-file.path(gout,"ldgeno")
    system2("plink",c("--bfile",BFILE,"--keep",keepfile,"--extract",idsfile,"--recode","A-transpose","--threads",Sys.getenv("SLURM_CPUS_PER_TASK","1"),"--out",pref),stdout=TRUE,stderr=TRUE)
    trawfile<-paste0(pref,".traw")

    if(!file.exists(trawfile)){
      cat("PLINK failed\n")
      master[[length(master)+1]]<-data.table(tissue,geneid=gene,status="PLINK_failed",n_locus=n_locus,n_shared=nrow(x),shared_fraction=shared_frac,n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }
    ...
    if(nrow(tr)<MINSHARED||!length(geno_cols)){
      ...
      master[[length(master)+1]]<-data.table(tissue,geneid=gene,status="too_few_PLINK_snps",n_locus=n_locus,n_shared=nrow(x),shared_fraction=shared_frac,n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }

    G<-as.matrix(tr[,..geno_cols]); storage.mode(G)<-"numeric"; orient<-x[match(tr$SNP,rawid),effect_allele]; flip<-toupper(tr$COUNTED)!=toupper(orient); G[flip,]<-2-G[flip,]
    G<-t(apply(G,1,function(v){v[is.na(v)]<-mean(v,na.rm=TRUE); v})); sdv<-apply(G,1,sd); ok<-is.finite(sdv)&sdv>0; tr<-tr[ok]; G<-G[ok,,drop=FALSE]; x<-x[match(tr$SNP,rawid)]

    if(nrow(x)<MINSHARED){
      cat("too few variable SNPs\n")
      master[[length(master)+1]]<-data.table(tissue,geneid=gene,status="too_few_variable_snps",n_locus=n_locus,n_shared=nrow(x),shared_fraction=nrow(x)/n_locus,n_candidate_SNPs=gm$n_candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p)
      next
    }

    R<-cor(t(G)); if(!is.matrix(R)||nrow(R)!=nrow(x)) stop(gene,": malformed LD matrix")
    ...
    s_eqtl<-tryCatch(susieR::estimate_s_rss(z1,R,n=N_EQTL),error=function(e) NA_real_); s_gwas<-tryCatch(susieR::estimate_s_rss(z2,R,n=GWAS_N),error=function(e) NA_real_)
    ...
    run_coloc_results<-function(S1,S2,x,gout){
      ...
      ret<-function(status,h4=NA_real_) list(status=status,h4=h4,ncs1=ncs1,ncs2=ncs2,eqtl_top=if(!is.na(top1))x$snpid[top1] else NA_character_,eqtl_pip=if(!is.na(top1))pip1[top1] else NA_real_,gwas_top=if(!is.na(top2))x$snpid[top2] else NA_character_,gwas_pip=if(!is.na(top2))pip2[top2] else NA_real_)
    ...
    },error=function(e) list(status=paste0("ERROR: ",conditionMessage(e)),h4=NA_real_,ncs1=NA_integer_,ncs2=NA_integer_,eqtl_top=NA_character_,eqtl_pip=NA_real_,gwas_top=NA_character_,gwas_pip=NA_real_))
    ...
    rec<-data.table(tissue,geneid=gene,status=ans$status,n_locus=n_locus,n_shared=nrow(x),shared_fraction=nrow(x)/n_locus,N_eQTL=N_EQTL,N_GWAS=GWAS_N,
                    n_candidate_SNPs=gm$n_candidate_SNPs,candidate_SNPs=gm$candidate_SNPs,best_candidate_SNP=gm$best_candidate_SNP,best_ALS_p=gm$best_ALS_p,
                    best_ALS_tissue_FDR=gm$best_ALS_tissue_FDR,best_ALS_global_FDR=gm$best_ALS_global_FDR,eqtl_LD_mismatch_s=s_eqtl,gwas_LD_mismatch_s=s_gwas,
                    n_eQTL_CS=ans$ncs1,n_GWAS_CS=ans$ncs2,top_eQTL_SNP=ans$eqtl_top,top_eQTL_PIP=ans$eqtl_pip,top_GWAS_SNP=ans$gwas_top,top_GWAS_PIP=ans$gwas_pip,
                    max_PP_H4=ans$h4,H4_reference_pass=!is.na(ans$h4)&ans$h4>=H4REF)

    master[[length(master)+1]]<-rec
    cat(ans$status,", shared=",nrow(x),"/",n_locus," (",round(100*nrow(x)/n_locus,1),"%), candidate_SNPs=",gm$n_candidate_SNPs,", eQTL_CS=",ans$ncs1,", GWAS_CS=",ans$ncs2,", H4=",signif(ans$h4,3),"\n",sep="")
    rm(G,R,x,q); gc()
  }
  rm(full); gc()
}
...
# STATUS COUNT TABLE
# ==============================================================
status_counts<-if(nrow(res)) res[,.(n_loci=.N),by=status][order(-n_loci)] else data.table(status=character(),n_loci=integer())
...
# ==============================================================
h4<-if(nrow(res)) res[is.finite(max_PP_H4),.(tissue,geneid,max_PP_H4,n_candidate_SNPs,best_candidate_SNP,best_ALS_p,best_ALS_global_FDR)] else data.table()
h4file<-file.path(BASE,"MR","PPH4_values.tsv"); fwrite(h4,h4file,sep="\t")

if(nrow(h4)){
  p<-ggplot(h4,aes(x=max_PP_H4))+geom_histogram(binwidth=.05,boundary=0,closed="left")+geom_vline(xintercept=H4REF,linetype="dashed",linewidth=.8)+
    scale_x_continuous(limits=c(0,1),breaks=seq(0,1,.1))+labs(title=paste0("SuSiE colocalization PP.H4 distribution (n=",nrow(h4),")"),
    subtitle=paste0("Dashed line = ",H4REF," reference threshold"),x="Maximum PP.H4 per gene–tissue locus",y="Number of loci")+theme_bw(base_size=12)

  plotfile<-file.path(BASE,"MR","PPH4_histogram.png"); ggsave(filename=plotfile,plot=p,width=8,height=5,dpi=300)
