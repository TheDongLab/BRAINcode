###########################################
# Rscript to run interaction cQTL analysis using Matrix eQTL
# Matrix eQTL by Andrey A. Shabalin
# http://www.bios.unc.edu/research/genomic_software/Matrix_eQTL/
# Usage: Rscript $PIPELINE_PATH/_cQTL_LINEAR_CROSS.R snp.txt expression.txt cov.txt output.txt circloc.txt snploc.txt
# Author: Xianjun Dong (modified by Zachery Wolfe)
# Version: 1.5 (Interaction Filter Edition - cQTL Version)
# Date: 2026-07-14
###########################################
...
circ_location_file_name = args[5] # Replaced gene_location
...
message("## Load circular RNA expression data...")
gene = SlicedData$new(); # Container kept as 'gene' for MatrixEQTL internal engine compatibility
gene$fileDelimiter = "\t";      
...
{
    message("## Load circRNA/SNP location data...")
    snpspos = read.table(snp_location_file_name, header = TRUE, stringsAsFactors = FALSE);
    genepos = read.table(circ_location_file_name, header = TRUE, stringsAsFactors = FALSE);
    ...
    cat('Detected local interaction cQTLs:', '\n');
    show(me$cis$eqtls)
    cat('Detected distant interaction cQTLs:', '\n');
    show(me$trans$eqtls);
} else {
    ...
    cat('Analysis done in: ', me$time.in.sec, ' seconds', '\n');
    cat('Detected interaction cQTLs:', '\n');
    show(me$all$eqtls);
}
