...
GENE_FILTER="C9orf72,UNC13A,ATXN1,ATXN2,FUS,SOD1,SOD1P1,TARDBP,TARDBPP2,HOMER1,RIMS1,RIMS2,CNTN1,SLC8A1,MAP7,ERC1,STMN2,UTRN"
...
def format_max_sig(v,sig=5):
    if v==0:return "0"
    d=max(0,sig-int(math.floor(math.log10(abs(v))))-1)
...
def load_location_table(path,kind,TYPE):
    ...
    rows=[]
    for x in lines:
        x+=[""]*max(0,len(headers)-len(x));rows.append(dict(zip(headers,x)))
    if kind=="snp":
...
'''
        ...
        try:subprocess.run(["bash","-lc",cmd],check=True)
...
def load_reference_exons(gtf,chrom,start,end):
    ...
    with open(gtf) as f:
        for l in f:
            ...
            a=parse_gtf_attrs(x[8]);tid=a.get("transcript_id","")
            if not tid:continue
            out.append({"start":s,"end":e,"strand":x[6],"gene":a.get("gene_name",a.get("gene_id","Unknown")),"gene_id":a.get("gene_id",""),"transcript_id":tid,"transcript_name":a.get("transcript_name","")})
    return out
def draw_reference_track(ax,exons,start,end):
    transcripts=defaultdict(list);info={}
    for e in exons:
        tid=e["transcript_id"];transcripts[tid].append((e["start"],e["end"]))
        info[tid]={"gene":e["gene"],"strand":e["strand"],"transcript_name":e["transcript_name"]}
    if not transcripts:
        ax.text(.5,.45,"No GENCODE v49 transcripts",transform=ax.transAxes,ha="center",fontsize=9);ax.set_ylim(-.45,.55)
    else:
        order=sorted(transcripts,key=lambda tid:(min(s for s,e in transcripts[tid]),info[tid]["gene"],tid))
        for y,tid in enumerate(order):
            ints=sorted(transcripts[tid]);tx_start=max(start,min(s for s,e in ints));tx_end=min(end,max(e for s,e in ints))
            ax.plot([tx_start,tx_end],[y,y],color="black",lw=.7)
            for s,e in ints:
                s=max(s,start);e=min(e,end)
                if e>s:ax.add_patch(Rectangle((s,y-.14),e-s,.28,facecolor="black",edgecolor="black",lw=.35))
            gene=info[tid]["gene"];strand=info[tid]["strand"];tname=info[tid]["transcript_name"]
            label=f"{gene} | {tname} | {tid} ({strand})" if tname and tname!=tid else f"{gene} | {tid} ({strand})"
...
with open(METADATA,newline="") as f:
    r=csv.DictReader(f);meta_headers,metadata=r.fieldnames or [],list(r)
...
def load_qtl_hits(path,TYPE):
    with open(path) as f:
        first=f.readline();d=detect_delimiter(first);headers=split_line(first,d);rows=[]
        for line in f:
            if not line.strip():continue
            x=split_line(line,d);x+=[""]*max(0,len(headers)-len(x));rows.append(dict(zip(headers,x)))
    event_aliases={"eQTL":["geneid","gene_id","gene","probeid","phenotype_id","id"],"sQTL":["junction_id","junctionid","splicing_id","splice_id","event_id","phenotype_id","probeid","id"],"cQTL":["circ_id","circid","circrna_id","event_id","phenotype_id","probeid","id"]}
    event_col=find_header(headers,event_aliases[TYPE]);snp_col=find_header(headers,["snp","snpid","snp_id","rsid","variant","variant_id","topSNP","lead_snp"])
    symbol_col=find_header(headers,["gene_symbol","symbol","gene_name"],False);p_col=find_header(headers,["pvalue","p_value","pval","p","pvalue_qtl"],False);fdr_col=find_header(headers,["fdr","qvalue","q_value","padj","adj_p"],False)
    hits=[];seen=set()
    for r in rows:
        event=str(r.get(event_col,"")).strip();snp=str(r.get(snp_col,"")).strip()
        if missing(event) or missing(snp):continue
        key=(event,snp)
...
def process_tissue(tissue):
    ...
    for i,hit in enumerate(hits,1):
        ...
        print(f"\n[{i}/{len(hits)}] {tissue} | {event} | {symbol} | {snp}"+(f" | p={qtl_p:.3e}" if qtl_p is not None else "")+(f" | FDR={qtl_fdr:.4g}" if qtl_fdr is not None else ""))

        phenotype=find_matrix_row(pheno_rows,event);genotype=geno_rows.get(snp)
        if phenotype is None or genotype is None or snp not in snp_locs:
            print("  SKIP: phenotype/genotype/SNP location missing");total_skipped+=1;continue
        ...
        if allele is None or allele["counted"] not in {allele["ref"],allele["alt"]}:
            print("  SKIP: allele orientation unavailable");total_skipped+=1;continue
        ...
        x=np.linspace(pstart,pend,nb,endpoint=False)+binbp/2
        exons=load_reference_exons(GTF,chrom,pstart,pend);n_transcripts=len({e["transcript_id"] for e in exons if e["transcript_id"]})
        ...
        for qid in phenotype.keys()&genotype.keys():
            ...
            if sd is None:
                print(f"    SAMPLE DIR MISSING: {qid} | {sid} | tissue={tissue}");continue
            records.append({"qtl_id":qid,"subject_id":subj,"sample_id":sid,"sample_fs_id":sd.name,"genotype":g,"genotype_raw":genotype[qid],"phenotype":ph,"is_als":status.get(qid,0),"sample_dir":sd,"primary_reads":None,"plus_track":None,"minus_track":None,"plus_source":None,"minus_source":None,"circ_reads":None,"circ_rpm":None,"linear_reads":None,"circ_percent_file":None,"circ_quant_source":None,"circ_match_type":None,"circ_matched_start":None,"circ_matched_end":None})
        ...
        if not complete:
            print("  SKIP: no subjects with usable strand-specific tracks");total_skipped+=1;continue
        ...
        with open(tsv,"w",newline="") as f:
            ...
            w.writerow(["tissue","qtl_type","event","gene_symbol","snp","qtl_p","qtl_fdr","ref","alt","plink_counted_allele","qtl_id","subject_id","metadata_sample_id","filesystem_sample_id","genotype_group","genotype_label","genotype_raw","phenotype","is_als","primary_reads","plus_track_source","minus_track_source","circ_reads","circ_rpm","linear_reads","circ_percent_file","circ_match_type","circ_matched_start","circ_matched_end","circ_quant_source","qtl_result_source"])
            for r in complete:
        ...
        ref_ratio=max(.72,min(4.0,.18*max(1,n_transcripts)));fig_height=max(10,9+.22*n_transcripts)
        ...
        if qtl_p is not None:title+=f" | QTL p={qtl_p:.3e}"
        if qtl_fdr is not None:title+=f" | FDR={qtl_fdr:.4g}"
        ...
        footer=(f"Source QTL result: {result_file.name} | Trait: {chrom}:{tstart:,}-{tend:,} | flank: {flank:,} bp each side | shaded region = trait locus | dashed vertical line = {snp} | coverage bin width ≈ {round(binbp)} bp\n"
