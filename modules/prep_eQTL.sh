...
SEX_MISMATCH_FILE=$OUTDIR/potential_sex_mismatches.txt   ### check your PCA output from _normQC.R BEFORE automatically removing these samples!

# --- OLD AUTOSOME-ONLY INPUTS (HASHED OUT) ---
# RAW=$PLINK/joint_autosomes_matrixEQTL.raw
# BIM=$PLINK/joint_autosomes_filtered_bed.bim

# --- NEW ALL-CHROMOSOME INPUTS (ACTIVE) ---
...
def clean_snp_id(full_id):
    ...
    if len(parts) >= 2:
        # Avoid prepending 'chr' if it is already present (e.g., 'chrX:10050')
