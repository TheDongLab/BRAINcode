#!/usr/bin/env bash
#SBATCH --job-name=cQTL_MR
#SBATCH --output=/home/zw529/donglab/data/target_ALS/MR/cQTL_MR.out
#SBATCH --error=/home/zw529/donglab/data/target_ALS/MR/cQTL_MR.err
...
# ==============================================================
# 1. BUILD SNP×circRNA EFFECT TABLE + UNIQUE-SNP STATISTICAL TABLE
# ==============================================================
for tissue in TISSUES:
    infile=f"{ROOT}/{tissue}/{tissue}_SNP_circRNA_MR_ready.tsv.gz"
    ...
    req=["snpid","circ_id","beta","se_qtl","p-value","gwas_beta_harmonized","gwas_se","gwas_p"]
    ...
    d=d.rename(columns={"beta":"beta_cqtl","p-value":"p_cqtl"})
    d["tissue"]=tissue
    d["F_statistic"]=(d["beta_cqtl"]/d["se_qtl"])**2

    # Input should already be harmonized + F>=10, but enforce it again.
    good=(d["F_statistic"]>=MIN_F)&d["beta_cqtl"].notna()&(d["beta_cqtl"]!=0)&(d["se_qtl"]>0)&d["gwas_beta_harmonized"].notna()&(d["gwas_se"]>0)&d["gwas_p"].notna()
    d=d[good].copy().drop_duplicates(["snpid","circ_id"])

    # ----------------------------------------------------------
    # Wald ratio is EFFECT-SIZE ANNOTATION ONLY.
    # No SNP×circRNA MR P-value is calculated.
    # ----------------------------------------------------------
    d["beta_Wald_ratio"]=d["gwas_beta_harmonized"]/d["beta_cqtl"]
    d["se_Wald_first_order"]=d["gwas_se"]/d["beta_cqtl"].abs()
    d["se_Wald_delta"]=np.sqrt(
        d["gwas_se"]**2/d["beta_cqtl"]**2 +
        d["gwas_beta_harmonized"]**2*d["se_qtl"]**2/d["beta_cqtl"]**4
    )
    ...
    # Check ALS statistics are identical across all circRNA rows.
    # ----------------------------------------------------------
    ...
    if len(bad): raise ValueError(f"{tissue}: {len(bad)} SNPs have inconsistent GWAS statistics across circRNA annotations")

    snp=d.sort_values(["snpid","p_cqtl"]).drop_duplicates("snpid").copy()
    ...
    annot=d.groupby("snpid").agg(
        n_circRNA_targets=("circ_id","nunique"),
        regulatory_circRNAs=("circ_id",unique_join)
    ).reset_index()
    ...
    summaries.append({
        "tissue":tissue,
        "SNP_circRNA_relationships":len(d),
        "unique_SNPs":len(snp),
        "target_circRNAs":d["circ_id"].nunique(),
        "multi_target_SNPs":int((snp["n_circRNA_targets"]>=2).sum()),
        "ALS_nominal_P0.05_SNPs":int((snp["gwas_p"]<.05).sum()),
        "ALS_tissue_FDR0.05_SNPs":int((snp["FDR_ALS_tissue"]<.05).sum())
    })

    print(f"SNP→circRNA relationships : {len(d):,}")
    print(f"Unique SNPs               : {len(snp):,}")
    print(f"Target circRNAs           : {d['circ_id'].nunique():,}")
    print(f"Multi-target SNPs         : {(snp['n_circRNA_targets']>=2).sum():,}")
...
gg=all_rel.groupby("snpid").agg(
    n_unique_circRNAs=("circ_id","nunique"),
    regulatory_circRNAs_all_tissues=("circ_id",unique_join)
).reset_index()
...
for tissue in TISSUES:
    ...
    rel=rel.merge(
        snp[["snpid","n_circRNA_targets","FDR_ALS_tissue","FDR_ALS_global"]],
        on="snpid",how="left",validate="many_to_one"
    )
    rel=rel.sort_values(["FDR_ALS_global","gwas_p","snpid","circ_id"])

    # 1 row per SNP: primary statistical results
    snp.to_csv(f"{out}/{tissue}_cQTL_SNP_results.tsv.gz",sep="\t",index=False,compression="gzip")

    # 1 row per SNP×circRNA: regulatory + Wald effect-size annotations
    rel.to_csv(f"{out}/{tissue}_cQTL_SNP_circRNA_effects.tsv.gz",sep="\t",index=False,compression="gzip")

# Global unique-SNP result table
global_snp.to_csv(f"{GLOBAL}/all_tissues_cQTL_SNP_results.tsv.gz",sep="\t",index=False,compression="gzip")

summary=pd.DataFrame(summaries)
summary.to_csv(f"{GLOBAL}/cQTL_MR_summary_all_tissues.tsv",sep="\t",index=False)

print(f"\n{'='*70}\nALL-TISSUE cQTL SNP-FIRST SUMMARY\n{'='*70}")
...
echo
echo "cQTL SNP-first analysis complete."
column -t -s $'\t' "$GLOBAL/cQTL_MR_summary_all_tissues.tsv"
