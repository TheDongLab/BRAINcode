...
###########################################
# _sQTL_manhattan.R
# ADDED: Interaction-mode baseline direction mapping to fix red-wall bias
###########################################
...
prepare_coords <- function(df) {
    ...
    # Run dynamic conditional check for ChrX vs Autosomes
...
# Determine if this is a dedicated standalone Chromosome X run
...
if (run_type == "interaction" && !is.null(std_path) && file.exists(std_path)) {
    message("## Interaction mode detected. Fetching baseline sQTL directionality for coloring...")
    
    std_df <- fread(std_path)
    if("gene" %in% names(std_df)) setnames(std_df, "gene", "junction_id")
    
    # Generate unique keys to safely map variant-junction pairs
    snp_plot[, match_key := paste0(snpid, "_", junction_id)]
    lead_plot[, match_key := paste0(snpid, "_", junction_id)]
    std_df[, match_key := paste0(snpid, "_", junction_id)]
    ...
    # Fall back to interaction beta sign if pair was missing in standard output
    snp_plot[is.na(color_beta), color_beta := beta]
    lead_plot[is.na(color_beta), color_beta := beta]
}

# ========================================================================
# PLOT 1: DIRECTIONAL COLORING (PSI CHANGE / BETA-BASED)
# ========================================================================
...
p1 <- ggplot(snp_plot, aes(x=cum_pos, y=log10p)) +
    ...
    labs(title=paste("cis-sQTL Manhattan Plot -", basename(out_prefix)), 
         subtitle=paste("Reds: Increased Intron Splicing (PSI) | Blues: Decreased Intron Splicing (PSI) | FDR <", sig_thresh_fdr), 
