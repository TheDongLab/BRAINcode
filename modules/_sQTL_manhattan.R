...
###########################################
# _sQTL_manhattan.R
# UPDATED: Generates both directional and junction-colored diagnostic plots
# EXCLUDES: Chromosome X/23 explicitly (Autosomes 1-22 only)
###########################################
...
# ========================================================================
# PLOT 2: DIAGNOSTIC COLORING BY UNIQUE JUNCTION ID
# ========================================================================
message("## Setting up Junction ID diagnostic colors...")
...
message("## Creating Junction ID Diagnostic Plot...")
p2 <- ggplot() +
    ...
    # 2. Significant points colored dynamically by their distinct Intron Junction ID string
    geom_point(data=snp_sig, aes(x=cum_pos, y=log10p, colour=junction_id), size=0.6, alpha=0.7) +
    
    # 3. Lead Diamonds mapped to the same Junction ID color scheme
    ...
    # Control the background layers using fill, leaving scale_colour to handle junctions dynamically
    scale_fill_manual(values=c("0"="#CCCCCC", "1"="#E5E5E5"), guide="none") +
    
    scale_x_continuous(labels=paste0("chr", snp_axis$chr_num), breaks=snp_axis$centre, expand=c(0.01, 0.01)) +
    scale_y_continuous(expand=c(0.02, 0.5)) +
    
    labs(title=paste("Diagnostic sQTL Manhattan Plot (By Splicing Locus) -", basename(out_prefix)),
         subtitle=sprintf("Every unique significant Junction ID is assigned an independent color | N Significant Introns = %d", uniqueN(snp_sig$junction_id)),
...
message(paste("## Junction diagnostic plot saved to:", out_file_png2))
message("## Done!")
