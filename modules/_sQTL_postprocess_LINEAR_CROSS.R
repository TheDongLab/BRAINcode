#!/usr/bin/env Rscript
###########################################
# _sQTL_postprocess_LINEAR_CROSS.R
# Post-processing script for interaction sQTLs
# (Adapted for Matrix eQTL LINEAR_CROSS mode)
###########################################
...
if (length(args) < 4) {
    stop("Usage: Rscript _sQTL_postprocess_LINEAR_CROSS.R <cis_file> <snp_loc_file> <gene_loc_file> <out_prefix> [fdr_thresh] [top_n]")
}
...
if (nrow(sqtl_fdr) == 0) {
  ...
  sqtl_ordered <- sqtl[order(`p-value`)]
  sqtl_fdr <- head(sqtl_ordered[`p-value` < 0.01], 250)
}

# Lead interaction SNP selection (per unique intron junction)
...
message(sprintf("## Total significant interaction pairs: %d", nrow(sqtl_fdr)))
message(sprintf("## Selecting ALL significant interaction pairs for boxplots (FDR < %s)...", fdr_thresh))
...
message(sprintf("## Total interaction pairs to plot: %d", nrow(sqtl_top)))
...
########################################################################
# Meta-Analysis Logic (Fixed Effects Inverse-Variance Method for Interactions)
########################################################################
...
if (file.exists(other_sex_file)) {
    ...
    message(sprintf("## Performing Inverse-Variance Fixed-Effects meta-analysis on %d shared interaction pairs...", nrow(meta)))
    
    # Calculate Standard Errors from interaction beta and t-stat (SE = |beta / t-stat|)
    ...
    meta_out <- file.path(combined_dir, paste0(tissue_name, "_Combined_Meta_Interaction_sQTL.txt"))
