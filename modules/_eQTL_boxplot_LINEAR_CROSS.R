#!/usr/bin/env Rscript
###########################################
# _eQTL_boxplot_LINEAR_CROSS.R
# Interaction Layout: Split Boxplots + Regression Slopes
###########################################
...
if (length(args) < 7) {
  stop("Usage: Rscript _eQTL_boxplot_LINEAR_CROSS.R <pairs_file> <snp_file> <expr_file> <cov_file> <snp_loc> <out_dir> <tissue>")
}
...
# Read file structure (expecting headers/columns: geneid, gene_symbol, snpid)
...
run_plotting <- function(pdf_path, use_status_colors = FALSE) {
    ...
    for (i in seq_len(nrow(pairs))) {
        ...
        # Super-title label
        mtext(paste("Gene:", sym_id, "(", ens_id, ") | SNP:", S), outer=TRUE, cex=1.1, font=2, line=0.5)
...
message(paste("# Processing", nrow(pairs), "interaction gene-SNP pairs..."))
