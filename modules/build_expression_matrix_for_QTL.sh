...
# safe mapping (no overwrite bugs)
wgs_map = (
    wgs.dropna(subset=["externalsubjectid", "externalsampleid"])
       .drop_duplicates("externalsubjectid")
       .set_index("externalsubjectid")["externalsampleid"]
       .to_dict()
)
...
print("samples used:", len(used_samples))
...
geno_header = pd.read_csv(geno_raw, sep=r"\s+", nrows=0)
geno_samples = [norm(x) for x in geno_header.columns[6:]]

rna_found["geno_id"] = rna_found["externalsubjectid"].map(wgs_map)
rna_found = rna_found.dropna(subset=["geno_id"])

rna_found["geno_id"] = rna_found["geno_id"].map(norm)
rna_found["externalsampleid"] = rna_found["externalsampleid"].map(norm)
...
print("Final expression matrix shape:", expr.shape)
